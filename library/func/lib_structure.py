...
def build_html_tree(package_name):
    """
    指定したパッケージのモジュール・クラス・関数・メソッドの構造を
    折り畳み可能なHTMLツリー形式で返す。
    """
            ...
            try:
                ...
                for name, obj in items:
                    ...
                    if inspect.isclass(obj):
                        # クラスのメソッドを抽出
                        html_lines.append(f"<li><details><summary><b>{safe_name}</b></summary><ul>")
                        methods = inspect.getmembers(obj, inspect.isfunction)
                        for method_name, _ in sorted(methods, key=lambda x: x[0]):
                            safe_method_name = html.escape(method_name)
                            html_lines.append(f"<li>{safe_method_name}</li>")
                ...
                html_lines.append(f"<li><b>{modname}</b> - Error: {html.escape(str(e))}</li>")
