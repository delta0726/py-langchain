...
# ＜概要＞
# - LLMの知識を補完したり専門的な処理をするため、LLMにツールを与えることができる
# - 例えば検索エンジンのツールを持たせると、リアルタイム情報にアクセスできるようになる
# - RAGによる知識の補完の限界を補うことができる

# ＜知識を与えることの限界＞
# - 情報の鮮度： 最新情報へのアクセスには限界がある
# - 網羅性の限界： あらゆる質問に対応することは難しい
# - ドメイン特化の難しさ： 専門知識として網羅することは難しい
...
#   --- 1回目のinvoke()では、ツールの選択と検索キーワードの提示を行う
#   --- 2回目のinvoke()では、実際に検索を行う
