...
# ＜Stateとは＞
# - グラフに処理結果の情報を管理/共有するものでノードやエッジをつなぐ裏付けとなる
#   --- これにより、処理結果に応じた分岐をエッジで表現することが可能となる


# ＜Stateの定義＞
# - TypeDictクラスを継承してStateクラスを辞書のような情報管理の処理ができるようにする
# - {count}でチャットボットとの対話した回数を記録する
# - {messages}はチャット履歴やエージェント同士のやり取りが格納される
#   --- Anotatedは型ヒントに追加のメタデータを付与するために使用される特殊な型のクラス
#   --- listとadd_messagesを組み合わせることで会話履歴をリストで管理する
#   --- TypedDictを使うことで順序に依存せずに変数名に基づいて管理する
