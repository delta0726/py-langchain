...
docs = collection.query(
    query_texts=[query], n_results=3, include=["documents", "distances", "embeddings"]
)

# 結果を表示 --- (*8)
docs0 = zip(docs["documents"][0], docs["distances"][0], docs["embeddings"][0])
for doc, dist, emb in docs0:
    print(f"(距離:{dist:.1f}) {doc}", emb[:3], "...")
